...
import { upsertTagAction, type FormState } from "@/lib/actions/categories";
...
export function TagFormDialog({ tag }: { tag?: Tag }) {
  ...
  async function handleSubmit(formData: FormData) {
    setPending(true);
    const result = await upsertTagAction(initialState, formData);
    setPending(false);
    if (result.error) {
  ...
  return (
    <Dialog
      ...
      onOpenChange={(next) => {
        setOpen(next);
        if (next) setColor(tag?.color ?? PRESET_COLORS[0]);
      }}
    >
